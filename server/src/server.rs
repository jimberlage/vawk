...
pub async fn listen(
    ...
    web::Query(listen_msg): web::Query<Listen>,
) -> HttpResponse {
    match executor.send(listen_msg).await {
        Ok(Ok(connection)) => actix_web::HttpResponse::Ok()
...
pub async fn cancel(
    executor: web::Data<Addr<CommandExecutor>>,
    web::Json(cancel): web::Json<Cancel>,
) -> HttpResponse {
...
pub async fn set_line_index_filters(
    ...
    match set_line_index_filters
        .filters
        .map(|filters| parsers::parse_index_filters(&filters))
    {
        ...
        Some(Ok(index_filters)) => {
            match executor
                .send(command_executor::SetLineIndexFilters {
...
pub async fn set_line_regex_filter(
    ...
    match set_line_regex_filter
        .filter
        .map(|filter| parsers::parse_regex_filter(&filter))
    {
        // TODO: Give more of an update to the user here about what went wrong.
        Some(Err(_error)) => actix_web::HttpResponse::BadRequest().finish(),
        Some(Ok(regex_filter)) => {
            match executor
                .send(command_executor::SetLineRegexFilter {
...
pub async fn set_line_separators(
    ...
    match set_line_separators
        .separators
        .map(|separators| parsers::parse_field_separators(&separators))
    {
        // TODO: Give more of an update to the user here about what went wrong.
        Some(Err(_error)) => actix_web::HttpResponse::BadRequest().finish(),
        Some(Ok(separators)) => {
            match executor
                .send(command_executor::SetLineSeparators {
...
pub async fn set_row_index_filters(
        ...
        .map(|filters| parsers::parse_index_filters(&filters))
    {
        ...
        Some(Ok(index_filters)) => {
            match executor
                .send(command_executor::SetRowIndexFilters {
...
pub async fn set_row_regex_filter(
        ...
        .map(|filter| parsers::parse_regex_filter(&filter))
    {
        ...
        Some(Ok(regex_filter)) => {
            match executor
                .send(command_executor::SetRowRegexFilter {
...
pub async fn set_row_separators(
        ...
        .map(|separators| parsers::parse_field_separators(&separators))
    {
        ...
        Some(Ok(separators)) => {
            match executor
                .send(command_executor::SetRowSeparators {
