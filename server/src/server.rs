...
async fn events(executor: web::Data<sync::Mutex<CommandExecutor>>) -> impl actix_web::Responder {
    let connection = executor.lock().unwrap().connect();
...
pub async fn run() -> io::Result<()> {
    ...
    let protected_executor = web::Data::new(sync::Mutex::new(executor));
