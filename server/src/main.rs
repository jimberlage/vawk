...
async fn main() -> io::Result<()> {
    ...
    actix_web::HttpServer::new(move || {
        actix_web::App::new()
            ...
            .route(
                "/api/line-index-filters",
                web::put().to(server::set_line_index_filters),
            )
            .route(
                "/api/line-regex-filter",
                web::put().to(server::set_line_regex_filter),
            )
            .route(
                "/api/line-separators",
                web::put().to(server::set_line_separators),
            )
            .route("/api/listen", web::get().to(server::listen))
            .route(
                "/api/row-index-filters",
                web::put().to(server::set_row_index_filters),
            )
            .route(
                "/api/row-regex-filter",
                web::put().to(server::set_row_regex_filter),
            )
            .route(
                "/api/row-separators",
                web::put().to(server::set_row_separators),
            )
